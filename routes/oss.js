...
function sanitizeBucketKey(raw) {
    return (BUCKETKEYPREFIX + raw).toLowerCase().replace(/[^a-z0-9\-_]/g, '-');
}

router.get('/api/images', async function (req, res, next) {
    try {
        const bucketKey = sanitizeBucketKey(req.query.bucket_key);
...
router.get('/api/signedurl', async function(req, res, next) {
  try {
    ...
    const signedUrl = await getSignedURL(bucketKey, objectKey);
...
router.post('/api/images', formidable({ maxFileSize: Infinity }), async function (req, res, next) {
    try {
      ...
      const bucketKey = sanitizeBucketKey(req.query.bucket_key);
      console.log(`[oss/upload] bucket="${bucketKey}", file="${file ? file.name : 'MISSING'}", size=${file ? file.size : 0}`);
      if (!file) {
        res.status(400).send('The required field ("image-file") is missing.');
        return;
      }
      const obj = await uploadObject(file.name, file.path, bucketKey);
      console.log(`[oss/upload] Success: objectKey="${obj.objectKey}"`);
      res.json({
          name: obj.objectKey,
